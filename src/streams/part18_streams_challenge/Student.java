...
public class Student {
    ...
    */
    ...
    /* getRandomSelection()
     * Randomize course each student gets
     * Takes a variable argument of courses & returns an array of courses
     * Get a random between 1 and the length of the courses passed plus 1
     * Create a modifiable Array list of the courses
     * Shuffle to change the order of the courses each time
     * Get a sublist of that starting from the 1st element upto the randomly
     * generated courseCount
     * Call this function in the getRandomStudent() b4 instantiating a new student
     * update instances of courses to randomCourses in getRandomStudent()
     */
    private static Course[] getRandomSelection(Course...courses){
        int courseCount = random.nextInt(1,courses.length + 1);
        var courseList = new ArrayList<>(Arrays.asList(courses));
        Collections.shuffle(courseList);
        var selectedCourses = courseList.subList(0,courseCount);
